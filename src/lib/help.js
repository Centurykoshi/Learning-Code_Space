...
export default function OrderByChat() {
    ...
    const handleClick = (title: string) => {
        if (["LineGraph", "BarGraph", "PieChart", "AreaChart"].includes(title)) {
            setSelectedGraph(title);
            toast.success(`Switched to ${title.replace("Graph", " Chart")}`);
        } else if (["7 days", "14 days", "Month", "Max"].includes(title)) {
            setSelectedPeriod(title);
            const periodText = title === "Max" ? "all data" : `last ${title.toLowerCase()}`;
            toast.success(`Showing ${periodText}`);
        }
    }
